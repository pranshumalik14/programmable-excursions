...
# ╔═╡ 438ea9dc-ef13-11eb-2a57-f74420be577f
begin
	include("Documents/projects/programmable-excursions/object_scanning/misc_utils.jl");
	using Plots;
	using LinearAlgebra;
	using StaticArrays;
	using Parameters;
...
# ╔═╡ 23469106-6645-4bac-b4d9-972a0c6a6491
using BenchmarkTools;

# ╔═╡ 2df7f9d2-80b4-40de-8239-c25311db8177
using PlutoUI;

# ╔═╡ 43b949b6-cfa4-4536-9ecd-828c8014f77d
...
# ╔═╡ 13b707cb-fdc9-4d8b-943f-a142834a0baf
function simulate_fwd!(ξᵣ, u, Δt, 𝒵=0, 𝒟=0, tol=1e-6)
	if abs(u.ω) > tol
...
# ╔═╡ 2dd25582-497a-47b0-84a7-9bf0f86f3b86
function simulate_fwd(ξᵣ, u, Δt, 𝒵=0, 𝒟=0, tol=eps())
	# ξₒ: ref pose at instantaenous center of curvature
...
# ╔═╡ 96e0279f-2fbc-45f8-bdba-517c1ff0d158
begin
	u₀ = [1.0, 0.0, 0.0]
	p  = [0.7, π/4]
	tspan = (0.0, 1.0)
	prob = ODEProblem(unicycle!, u₀, tspan, p)
	sol = solve(prob)
	plot(sol, vars=(1,2); aspect_ratio=:equal, label="traj")
end

# ╔═╡ 370ab921-b29f-42b5-9db7-4a600e2b4a24
begin
	ξᵣ = Pose2(u₀[1], u₀[2], u₀[3])
	ξ̂ᵣ = simulate_fwd(ξᵣ, (v=p[1], ω=p[2]), tspan[2])
	ξ̃ᵣ = simulate_fwd!(ξᵣ, (v=p[1], ω=p[2]), tspan[2])
	plot(sol, vars=(1,2); aspect_ratio=:equal, label="Trajectory")
	plot_pose(ξᵣ; color="green", label="Start", legend=true)
	plot_pose(ξ̂ᵣ; color="red", label="End", legend=true)
	plot_pose(ξ̃ᵣ; color="orange", legend=true)
end
...
git-tree-sha1 = "0fad303d5f06f198ab2a1e9db0d3f8e452c2e869"
